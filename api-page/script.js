...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const populatePageContent = () => {
        ...
        const creator = settings.apiSettings?.creator || 'venn';

        setPageContent(DOM.pageTitle, settings.name, "venncode API");
        setPageContent(DOM.wm, `© ${currentYear} ${creator}. Semua hak dilindungi.`);
        setPageContent(DOM.appName, settings.name, "vc-API");
        setPageContent(DOM.sideNavName, settings.name || "API");
        setPageContent(DOM.versionBadge, settings.version, "v1.2A");
        ...
        if (DOM.apiLinksContainer) {
            ...
            const defaultLinks = [{ url: "https://venncode-inc.github.io/Fp/", name: "Akun Admin", icon: "fab fa-github" }];
